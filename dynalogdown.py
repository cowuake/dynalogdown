...
def main():
    try:
        welcome()
        config = read_config()
        [time_from, time_final] = [datetime.fromisoformat(time).astimezone(ZoneInfo(config.time_zone))
        ...
        while True:
            while True:
                print(f'[{EMOTION_INDIFFERENT}] Searching in the interval ' +
                ...
                if n_entries < 250 and time_to + config.magic_factor * dt < time_final:
                    print(f'[{EMOTION_PLEASED}] Found {n_entries} < 250 log entries within the time interval. Increasing delta.')
                ...
                print(f'[{EMOTION_UNCERTAIN}] Found {n_entries} > 1000 log entries within the time interval. Reducing delta.')
                dt /= 2
                time_to = time_from + dt
            ...
            print(f'[{EMOTION_HAPPY}] Written {n_entries} log entries on \'{config.file}\'.')
...
def init_log_file(config):
    ...
    lines = [line.ljust(n_columns + hspace, ' ').rjust(n_columns + 2 * hspace, ' ') for line in lines]
    lines = [line.ljust(n_columns + 2 * hspace + hfill, PADDING_CHAR).rjust(n_columns + 2 * (hspace + hfill), PADDING_CHAR) for line in lines]
...
def validate_response(response):
    if response.status_code != 200:
        say_goodbye(f'[{EMOTION_UNHAPPY}] Could not fulfill the job: the server returned {response.status_code} ({response.reason}).')

def count_log_lines(text, source_name):
    pattern = r"(?P<leading>.*?)" + re.escape(f"\"{source_name}\"") + r":\s?(?P<number>\d+)(?P<trailing>.*?)"
